...
class Job51Spider(scrapy.Spider):
    ...
    def positionList(self, response):
        print(response.meta.get('city_name'))
    ...
    def positionDetail(self, response):
        ...
        item_loader.add_value('location',
                              re.sub('\t', '', response.css(".tBorderTop_box:nth-child(3) p::text").extract()[1]))
    ...
    def companyDetail(self, response):
        ...
        labels_part = re.sub(r'\s+', '', response.css(".tHeader.tHCop .in p::text").extract_first()).split('|')
        item_loader.add_value('size', labels_part[1])
        item_loader.add_value('company_nature', labels_part[0])
        item_loader.add_value('industry', labels_part[2])

        info_part = re.sub('\s+', '', ''.join(response.css('.tBorderTop_box.bmsg .inbox p::text').extract()))
